...
int encode_ue_network_capability(
    UENetworkCapability uenetworkcapability, uint8_t iei, uint8_t* buffer,
    uint32_t len) {
  uint8_t* lenPtr;
  uint32_t encoded = 0;
  int encode_result;
  ...
  if ((encode_result = encode_bstring(
           uenetworkcapability, buffer + encoded, len - encoded)) < 0)
...
int decode_ue_network_capability(
  ...
  if ((decode_result = decode_bstring(
           uenetworkcapability, ielen, buffer + decoded, len - decoded)) < 0)

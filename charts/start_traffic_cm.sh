...
do
	...
	do
		...
		dnnpod=$(kubectl get pods -n oai  | grep oai-dnn$gnbsimc | awk '{print $1}')
		...
		echo "$upfdata" | sed 's/[A-Za-z]*//g' >> logs/$usecase/compute/$aggr/compute_logs$gnbsimc.txt
		...
		echo "$dnndata" | sed 's/[A-Za-z]*//g' >> logs/$usecase/compute/$aggr/compute_logs$gnbsimc.txt
		...
		echo "$gnbsimdata" | sed 's/[A-Za-z]*//g' >> logs/$usecase/compute/$aggr/compute_logs$gnbsimc.txt
		...
		do
			...
			echo "$data" | sed 's/[A-Za-z]*//g' >> logs/$usecase/compute/$aggr/compute_logs$gnbsimc.txt
		done

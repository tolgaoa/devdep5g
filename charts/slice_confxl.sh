...
gnbsims=10
	...
	do
		...
		do
			...
			echo "Waiting Metrics Resolution Initial Refresh for 20s"
			sleep 20
			...
			do
				...
				sed -i "/version/c\  version: ngamingdu" charts/oai-gnbsim/values.yaml
				...
				if [ $a == 1 ]; then
					...
					#-----------------------------------------------------------------------------------------
					pod=$(kubectl get pods -n oai  | grep amf$amfc | awk '{print $1}')
					...
					echo "$data" | sed 's/[A-Za-z]*//g' >> logs/deployment/compute_logs$gnbsims.txt
					
					#----------------------------------------------------------------
					pod=$(kubectl get pods -n oai  | grep smf$amfc | awk '{print $1}')
					data=$(kubectl top pod -n oai $pod)
					k="0"
					while [ $k -lt 4 ]
					do
					...
					#---------------------------------------------------------------
					pod=$(kubectl get pods -n oai  | grep spgwu-tiny$amfc | awk '{print $1}')
					data=$(kubectl top pod -n oai $pod)
					k="0"
					while [ $k -lt 4 ]
					do
					...
					#----------------------------------------------------------------
                                        pod=$(kubectl get pods -n oai  | grep ausf$nrfc | awk '{print $1}')
                                        data=$(kubectl top pod -n oai $pod)
                                        k="0"
                                        while [ $k -lt 4 ]
                                        do
					...
					#----------------------------------------------------------------
                                        pod=$(kubectl get pods -n oai  | grep udm$nrfc | awk '{print $1}')
                                        data=$(kubectl top pod -n oai $pod)
                                        k="0"
                                        while [ $k -lt 4 ]
                                        do
					...
					#----------------------------------------------------------------
                                        pod=$(kubectl get pods -n oai  | grep udr$nrfc | awk '{print $1}')
                                        data=$(kubectl top pod -n oai $pod)
                                        k="0"
                                        while [ $k -lt 4 ]
                                        do
					...
					#----------------------------------------------------------------
					pod=$(kubectl get pods -n oai  | grep nrf$nrfc | awk '{print $1}')
					data=$(kubectl top pod -n oai $pod)
					k="0"
					while [ $k -lt 4 ]
					do
					...
					#----------------------------------------------------------------
                                        pod=$(kubectl get pods -n oai  | grep mysql$sqlc | awk '{print $1}')
                                        data=$(kubectl top pod -n oai $pod)
                                        k="0"
                                        while [ $k -lt 4 ]
                                        do
                                        ...
                                        #----------------------------------------------------------------
					canalpods=$(kubectl get pods -A  | grep canal | awk '{print $2}')
					for x1 in $(echo $canalpods); 
					do
						pod=$(echo $x1)
					...
					echo "Already gathered deployment compute logs in the first aggregration"
				fi
				...
				sed -i "26s/.*/        image: tolgaomeratalay\/oai-dnn:gaming/" charts/oai-dnn/02_deployment.yaml
	...
	/bin/bash ./charts/start_traffic.sh $trafficusers $a $rep

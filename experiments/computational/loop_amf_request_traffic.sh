...
initialize_output_file() {
    if [ ! -f "$1" ]; then
        echo "Timestamp,Response Time (ms)" > "$1"
    fi
}
...
make_amf_ausf_aka_request() {
    ...
    local time_ms=$(kubectl exec $amfpod -n $namespace -c amf -- curl -X POST "http://oai-ausf10-svc:80/nausf-auth/v1/ue-authentications" \
    -d '{"servingNetworkName":"5G:mnc095.mcc208.3gppnetwork.org","supiOrSuci":"208950000000010"}' \
    -H "Content-Type: application/json" \
    -w '%{time_total}' -o /dev/null -s | awk '{print $1*1000}')
    echo "$timestamp,$time_ms" >> "$amf_to_ausf_aka_outputFile"
}

make_amf_nrf_disc_request() {
    local timestamp=$(date +%Y-%m-%dT%H:%M:%S)
    local time_ms=$(kubectl exec $amfpod -n $namespace -c amf -- curl "http://oai-nrf10-svc:80/nnrf-disc/v1/nf-instances?target-nf-type=SMF&requester-nf-type=AMF" \
    -w '%{time_total}' -o /dev/null -s | awk '{print $1*1000}')
    echo "$timestamp,$time_ms" >> "$amf_nrf_disc_outputFile"
}

make_amf_pdu_session_request() {
    local timestamp=$(date +%Y-%m-%dT%H:%M:%S)
    local time_ms=$(kubectl exec $amfpod -n $namespace -c amf -- curl -X POST "http://oai-smf10-svc:80/nsmf-pdusession/v1/sm-contexts" \
    ...
    -H "Content-Type: application/json" \
    -w '%{time_total}' -o /dev/null -s | awk '{print $1*1000}')
    echo "$timestamp,$time_ms" >> "$amf_pdu_session_outputFile"
}

make_amf_smf_modify_request() {
    local timestamp=$(date +%Y-%m-%dT%H:%M:%S)
    local time_ms=$(kubectl exec $amfpod -n $namespace -c amf -- curl -X POST "http://oai-smf10-svc:80/nsmf-pdusession/v1/sm-contexts/1/modify" \
    -d '{"n2SmInfo":{"contentId":"n2msg"},"n2SmInfoType":"PDU_RES_SETUP_RSP"}' \
    -H "Content-Type: application/json" \
    -w '%{time_total}' -o /dev/null -s | awk '{print $1*1000}')
    echo "$timestamp,$time_ms" >> "$amf_smf_modify_outputFile"
}
